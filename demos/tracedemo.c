...
{
    // Delete cstrace.bin if it exists
    if (access("cstrace.bin", F_OK) == 0) {
        if (unlink("cstrace.bin") != 0) {
            perror("Failed to delete existing cstrace.bin");
        } else {
            printf("Deleted old cstrace.bin\n");
        }
    }
    ...
    if (itm) {
        // cs_trace_stimulus(devices.itm, 0, 0x010001);
        // cs_trace_stimulus(devices.itm, (1 & 0xF), 0xDEADBEEF);
        // for (i = 0; i < 2; ++i) {
        //     //cs_trace_stimulus(devices.itm, (i & 0xF), seed + i);
        //     // cs_trace_stimulus(devices.itm, (i & 0xF), 0xDEADBEEF);
        //     cs_trace_stimulus(devices.itm, 1, 0xDEAD);
        //     cs_trace_stimulus(devices.itm, 2, 0xBEEF);
        //     // usleep(100);
        // }
        // cs_trace_stimulus(devices.itm, 1, 0xDEADBEEF);
        // cs_trace_stimulus(devices.itm, 2, 0xDEADBEEF);
        // cs_trace_stimulus(devices.itm, 3, 0xDEADBEEF);

        while (cs_get_buffer_unread_bytes(devices.etb) < 256) {
            cs_trace_stimulus(devices.itm, 1, 0xDEADBEEF);
            usleep(100);
        }
